...
http://testphpagro1.ua/index.php?page=books
Array
(
    [route] => index.php
    [page] => services
...
$allowed = array('static', 'auth', 'comments', 'contacts', 'errors', 'game', 'goods', 'news','partners', 'books',
...
бы мог пригодиться в другом месте, где бы мы использовали идентификатор. Если бы номер категории нам был бы нужен,
...
...
Это все "один ко многим", так как один ноутбук не может иметь два разных экрана или разные производлители.
...
33.2 Основы JS
JS работает в html. PHP тоже может работать в HTML, но его мы можем открыть в любом месте HTML страницы.
...
Переменной может быть лбой набор символов, при условии, что оно не начинается с цифры. В JS var и $var это разные
переменные.
...
Глобальная переменная может біть переназначена внутри функции:
x = 5;
function calc(num1, num2, action) {
...
y = calc(1,5);
alert(x); //в этом месте тот же алерт уже выведет 6, так как отработала функция и используется переназначенная переменная
...
к его атрибутам ('id', 'style'). Мы можем ссылаться на какие-нибудь внутренние эелементы внутри него (text) или
...
JS должен срабатывать не на открытие страницы, а на определенные действия пользователя. При нажатии на каку-то галочку,
...
1) выолняется замена текста на Hello
2) скрытие блока.
...
Если в первом варианте работы JS мы использовали скрипты в тегах html, то во втором вариенте работу JS выносим отдельно.
...
Где можно использовать "return false"? Можно при работе с формой
<form action="http://google.com" method="get" onsubmit="return del()">
...
<script>
    function del() {
        l = document.getElementById('key').value.length; // в всплывающем окне напишет количество символов в запросе
        if(l < 5) {
