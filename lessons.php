...
чтобы в папке находилсь php, вордовские документы и т.п. Сюда можно еще отнести проверку на безопасность, не только на сами типы, но еще на соответствие размерам.
...
В Windows с правами доступа проблем нет, если мы пользуемся IDE с правами администратора. Для UNIX систем есть разграничения - у админа могут быть права на доступ к папке, а у приложения нет доступа, все зависит как будет установлена программа. Из-за этого возникает путаница прав.
...
Загрузка файлов не работает методом get, поэтому выбираем только post0 В форме выбираем enctype="multipart/form-data" для того, чтобы мы могли загрузить наши картинки.
...
(
    [file] => Array
        (
            ...
            [size] => 229357 // размер в байтах (1кБайт=1024байта)- будем делать тоже проверку чтобы не грузили большие или маленькие файлы
        )

)

Но после этого файл попадает во временную папку и если нам нужен этот файл, то мы должны после этого его скопировать в нашу папку, куда нам необходимо. Через время временная папка очищается.
Хакер может использовать тип файла [type] => image/jpeg выдав себя за изображение и загрузить php файл с каким-нибудь вредоносным скриптом, поэтому доверять этому типу нельзя! Мы будем проверять по другому.
...
для этой функции нужно указать имя файла, причем имя должно быть обязательно латинскими буквами, без спецсимволов и быть уникальным, то есть чтобы случайно два человека в одну и туже секунду не попытались загрузить файл с одинаковым названием, поэтому мы название будем переименовывать, чтобы максимально отпали совпадения. Для этого в название файла заложим: папку куда будем загружать + функцию времени + тип + случайное число + расширение .jpg :
...
При загрузке изображения расширение может быть написано как большими так и маленькими буквами, поэтому мы сразу через функцию делаем названия расширений маленькими буквами:
...
30.5 Изменение размера изображения (00:59:40):
Для этого нужно будет самому доделать наш скрипт  - домашка, а пока разберемся какие для этого существуют функции:
imagecreatefromjpeg
imagecreatefrompng
