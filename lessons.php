...
28.1.2. Массивами: ... count(); array_sum;
...
$pattern = '#шаблон, по которому идет поиск#свойства данного поиска'; //пишется как строка в кавычках
...
28.2.2 Свойства паттерна - это перечисление определенных параметров:
...
28.2.7 Дополнительные символы: (01:01:10)
1) ^ - Поиск от начала строки, ставится сразу после решетки:
$text = 'АввгдеЖ';
$pattern = '#^[а-е][в-я]#ui';
...
4) Сокращения количества: (01:23:10)
...
Если начнем искать букву "А" с начала строки, то выдаст не полноценные слова, а и части слов:
$pattern = '#А[а-яё]*#ui';
Array
(
    [0] => Array
...
Если в патерне указать, что первым символом перед "А" должна содржеться какая-то буква (меняем * на +), то в
...
6) Дополнительные сокращения: (01:34:10)
...
Но просто ставить пробле не лучший вариант, поэтому мы используем спецсимвол (\s):
$text = 'Вася                       любит ананасы, Петя любит кораблики. Пёсик самый лучший. А на юге живут папуасы';
...
7) . - абсолютно любой символ! (01:43:55):
