...
позволяет добавлять вспылывающие окна, добавление комментариев где угодно. У JS - динамическая работа страниц.
...
появится диалоговое окно, затем форма. Если после формы, то наоборот. Например:
...
<script>
    ...
    //получаем содержимое блока с id = 'xx'
    document.getElementById('xx').innerHTML = 'Какой-то текст'; //Это Set, так как мы переобозначаем
    //заменяем содержимое в блоке с id = 'xx' на 'Какой-то текст'
</script>
...
<script>
    document.getElementById('xx').style.display = 'none'; // скрываем блок через css
    //еще вариант: document.getElementById('xx').hidden = true; //скрываем блок через свойство
    alert(1);
    document.getElementById('xx').style.display = 'block'; // включаем отображение блока через css
    //еще вариант: document.getElementById('xx').hidden = false; //отмена скрытия через свойство.
...
В разработке используеются все три варианта. Первый вариант onclick в тегах очень полезен, когда мы не можем создать
...
Мы ранее обсуждали, что при удалении необходимо спросить подтверждение
...
Урок 37: Изучаем ajax и jQuery (2022-08-20)

37.1  Эффективность и необходимость jQuery
jQuery — это быстрая, лёгкая и многофункциональная JavaScript библиотека, основанная на принципе «пиши меньше,
делай больше».
...
Или скачать с офсайта и подключить как css.

Проверке на JS верить нельзя, так как всегда есть возможность пользователю через просмотр исходного кода удалить
подключение JS скрипта + редактировать теги и в итоге проверка не сработает. Проверки на PHP обойти нельзя, поэтому
если мы делаем проверки на JS, то это только для визуального впечатления посетителей, но за безопасность сайта
отвечает PHP.

37.2 Простые AJAX запросы и ответы (00:31:30)
AJAX – это аббревиатура от «Asynchronous JavaScript and XML», которая дословно переводится как «асинхронный JavaScript
и XML».
асинхронный – действие выполняется в фоне (не в основном потоке), другими словами, таким образом, что оно не мешает
пользователю взаимодействовать со страницей;
JavaScript – язык, на котором всё это делается (т.е. создание и настройка запроса, отправка его на сервер, получение
ответа и его разбор, обновление страницы);
XML – формат для хранения и передачи данных, в настоящее время вместо него чаще используется JSON, но кроме них можно
использовать и другие форматы.

AJAX – это термин, который описывает как можно используя существующие технологии получать данные с сервера в фоновом
режиме и использовать их для обновления страницы (без перезагрузки). Основная цель AJAX – это сделать сайты и
веб-приложения более удобными, быстрыми и отзывчивыми.

Пример работы ajax. Создаем страничку test_ajax.php. В ней пишем сообщение:
<?php
//echo 'это пришел ответ со страницы test_ajax.php';
//sleep(10);
if(empty($_POST['login'])) { //если все хорошо
    echo 'ok'; //выведем ок
} else {
...
foreach ($_POST as $k=>$v) {
    echo $k.' = '.$v.'<br>'; //вместо вывода на экран этим скриптом можно записать результат в БД
}
?>
<script>
    function myAjax() {
        //document.getElementById('yyy').innerHTML += '<br>Login = Inpost';
        x = document.getElementById('xxx').innerHTML; //добавляем вывод Тестируем AJAX в alert
        //Синтаксис jQuery начинается с функции jQuery или с более короткого и красивого псевдонима - знака $
        $.ajax({ //вызов метода ajax, в круглых скобках передаем аргумент, в фигурных - передали объект в виде
            // многомерного массива с какими-то параметрами - ключ: значение
            url: '/test_ajax.php', //обращение по url к этому файлу
            type: "POST", //указание способа передачи
            cache: false, //кеширование - это запоминание чего-то, в данно случае ответа, тут отключен кеш
            data: {login: x, age: 24, text: x},// {key1: 'value1', key2: 'value2'}//отправка на сервер
            timeout: 5000, //время ожидания запроса
            success: function (msg) {//ответ сервера в виде безымянной функции с аргументом msg
                if (msg == 'ok'){
                    //alert('Ваш комментарий был успешно добавлен')//вместо алерта можно добавить div с тектом:
                    document.getElementById('divlalala').innerHTML = msg;
                } else {
                    alert(msg);
                }
            },
            error: function (x, t, m) {//если за время timeout ответ не пришел с сервера, то:
                //alert(6);
                if(t==="timeout") {
                    alert('Ожидание ответа сервера слишком велико');
                } else {
...
window.onload = function () {
        document.getElementById('xxx').onclick = myAjax; //вызов функции myAjax при клике на div
}
</script>
...
<div id="yyy" style="color:#00C" data-color="green">
    Тестируем jQuery
</div>
</body>

AJAX при простой настройке, без дополнительных параметров, работает только внутри данного домена, с домена test1.ua мы
не сможем запросить какой-то другой домен.

В чатах кеширование отключено, так как с кешированием чат не будет обновляться. Кеширование включается для подгрузки
статичных элементов, которые не меняются при переходах страниц

Если прописать в php перед циклом sleep(10), то скрипт остановится на 10 секунд, за это время сработает timeout: 5000,
и alert ошибки


Урок 38: Углубляемся в ajax и jQuery, JSON для массивов (2022-08-21)
<!--План:
JSON как передача массивов
Селекторы на jQuery
Готовимся к настоящему программированию сайтов-->

38.1 JSON как передача массивов
Если в чате очень много сообщений, то у нас есть два варианта: 1) мы запросим все, 2) запросить только те комментарии,
которые появились после последнего посещения чата.
В таком случае нам помогут сессии:
<?php
session_start();
$_SESSION['time'] = time();//время последнего запроса
?>
Мы просим новые коментарии, где $_SESSION['time'] > time... который на сервере записан в БД больше, чем та дата, когда
в последний раз запрашивали новые комментарии. После чего мы получили новые комментарии и в цикле их обработали,
вывели на экран через echo - все у нас создался новый этап времени:
while() {echo ...}
$_SESSION['time'] = time();
таким образом мы можем подгружать новые комментарии, не трогая заново каждый раз всю нашу структуру, а значит обработка/
отправка данных будет гораздо быстрее. Это первая вещь.
Вторая вещь с ajax станет вопрос - я не хочу выводить весь текст тут, я хочу часть текста вывести в правом блоке, часть
текста в левом блоке, какой-то вверху и так далее, а в ajax все идет в одном потоке.
С этим можно решить на уровне php через отслеживание знака |:
<?php
$string =  '
    text для блока №1 | текст для блока №2 | лалала
';
explode('|', $string); //разбиение строки по знаку |
?>
Но это не лучший вариант, так как мы можем сразу приготовить готовый массив, который мы можем передать из php в JS:
<?php //создаем массив:
    $array = array (
        'name' => 'Alexandr',
        'last_name' => 'Radeonoff',
    ...
    $array['status'] = 'ok'; // если сменить статус с ок на empty или 0, то вывод алерта не произойдет.
/*выводим массив в виде JSON (JavaScript Object Notation) – это текстовый формат представления данных в нотации
 объекта JavaScript:*/
    ...
    function myAjax() {
        $.ajax({
            ...
            success: function (msg) {
                //получить массив в виде объекта и потом с этими данными работать
                var response = JSON.parse(msg); //перевод строки JSON в объект (данный процесс ещё называют парсингом);
                if (response.status == 'ok') {
                    alert(response.name + ', ' + response.last_name); //вывод имени из массива //Alexandr, Radeonoff
                }
...
38.2 Просмотр ajax через панель разработчика (00:14:20).
Для этого перейти в панель разработчика (ПКМ просмотреть код) дальше вкладка "Сеть", выбираем запрос ajax и здесь
можно просмотреть его содержание (заголоки запроса/ответа, данные методом POST, ответ сервера, предварительный
просмотр, инициатор, время и файлы куки).

38.3 Работа jQuery с DOM деревом / Селекторы на jQuery (00:16:45).
<script>
    window.onload = function () {
        document.getElementById('yyy').onclick = myJquery;
    }
    function myJquery() {
        //var x = $('#xxx').attr();//работа с атрибутами
        // это аналогично записи: document.getElementById('xxx').style.display = 'block';
        var x = $('#xxx').css('background-color'); //работа с css
        $('#xxx').css('background-color', 'red');//меняется цвет фона первого div на красный
        $(this).css('color', 'red');
        //nowtime = new Date().getTime();// возвращает сколько сечас времени
    }
</script>
<body>
<div id="xxx">
...
HTML-атрибуты – это дополнительные значения для установления различных характеристик HTML-элементам в HTML-коде.
С помощью них можно определённым образом настроить элемент и отрегулировать его поведение. Все атрибуты указываются
внутри открытого тега и отделяются друг от друга посредством пробела.
Какие у тега бывают атрибуты? 'id', 'data-...' .

В jQuery для выполнения действий над атрибутами имеется два метода:
1) attr() – используется, когда нужно, узнать значение требуемого атрибута, добавить к элементу новый атрибут или
изменить текущее значение некоторого атрибута на новое;
2) removeAttr() – применяется для удаления определённого атрибута у элемента или набора элементов.

get метод выглядит так:
var x = $('#yyy').attr();
set метод выглядит так:
$('#yyy').attr('id', 'SET');
меняем цвет через SET:
$('#yyy').attr('data-color', 'red');

Еще больше функций и эффектов jQuery можно найти на их сайте.
